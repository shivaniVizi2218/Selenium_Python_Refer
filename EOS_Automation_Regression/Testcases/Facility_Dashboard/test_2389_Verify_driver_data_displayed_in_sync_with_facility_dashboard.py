...
class Test_2389_testcase:
    EOSURL = ReadEOSurlConfig.getEOSuRL()
    testDataFile = open('.\\Data\\package_entry_search.json')
    data = json.load(testDataFile)

    def test_2389_status_verify(self, setup):
        self.driver = setup
        ...
        time.sleep(30)
        self.packageEntry.verify_delivered_status(route_number=self.data["route_number"])
        self.dispatch.click_on_CloseBtn()
        time.sleep(2)
        self.facility_dashboard.click_on_facility_dashboard()
        time.sleep(2)
        self.facility_dashboard.click_on_delivery_column()
        time.sleep(2)
        self.packageEntry.verify_delivery_column_vals(barcode)
