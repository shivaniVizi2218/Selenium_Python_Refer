...
#https://dev.azure.com/OnTracDevelopment/Holistic%20Software%20Destroyers/_workitems/edit/2462
...
class Test_TC_2462:
    ...
    def test_TC_2462(self, setup):
        ...
        if (self.advance_search.validate_package_status_to_be("Out For Delivery",
                                                              10, 20) != True):
            self.advance_search.change_status_as_outofDelevery(
                self.data["advance_package_window_status_Out_for_delivery_value"])
            self.advance_search.validate_package_status_to_be("Out For Delivery",
                                                              10, 10)
        ...
        notes_text = self.daily_closeout_dashboard_data["Attempt_Notes_text"] + stop_name_random.lower()
        self.dailyClouseout.select_one_attempt_option(notes_text)
        self.dailyClouseout.verify_facility_dashboard_deliveries_under_att_column(barcode)
