import json
import time
from eospageObjects.advance_search import eos_advance_search
from eospageObjects.package_entry import package_entry
from utilities.readProperties import ReadEOSurlConfig
from Testcases.configtest import setup
from eospageObjects.facility_dashboard import eos_facility_dashboard
from eospageObjects.bolt_menu import eos_bolt_menu
from eospageObjects.eos_manual_regression_only import eos_manual_regression
from eospageObjects.dispatch_dashboard import eos_dispatch_dashboard

from utilities import DB_connection
from utilities.common_util import generate_random_string
from utilities.readProperties import ReadEOSdbConfig


# https://dev.azure.com/OnTracDevelopment/Holistic%20Software%20Destroyers/_workitems/edit/2414

class Test_TC_2414:
    EOSURL = ReadEOSurlConfig.getEOSuRL()
    EOSserver = ReadEOSdbConfig.getEOSdb_server()
    EOSpassword = ReadEOSdbConfig.getEOSdb_pass()
    EOSuser = ReadEOSdbConfig.getEOSdb_user()
    testDataFile = open('.\\Data\\advance_search.json')
    data = json.load(testDataFile)
    bolt_data_file = open(".\\Data\\bolt_menu.json")
    bolt_data = json.load(bolt_data_file)

    def test_TC_2414(self, setup):
        self.driver = setup
        self.advance_search = eos_advance_search(self.driver)
        self.packageEntry = package_entry(self.driver)
        self.facility_dashboard = eos_facility_dashboard(self.driver)
        self.bolt_menu_dashboard = eos_bolt_menu(self.driver)
        self.dispatch_dashboard = eos_dispatch_dashboard(self.driver)
        self.eos_manual_regression_page = eos_manual_regression(self.driver)
        self.driver.get(self.EOSURL)
        self.driver.maximize_window()
        self.driver.implicitly_wait(20)

        query = "SELECT * FROM [dbo].[Facility]WHERE FacilityName LIKE 'West Palm Beach'"
        facility = DB_connection.execute_query(self.EOSserver, "PackageDB",
                                               self.EOSuser, self.EOSpassword,
                                               query)
        facility_id_number = facility[0][0]
        print(facility_id_number)
        query2 = f"SELECT * FROM Route WHERE FacilityId = {facility_id_number} AND Active = 1"
        route = DB_connection.execute_query(self.EOSserver, "PackageDB",
                                            self.EOSuser, self.EOSpassword,
                                            query2)
        facility_route = route[0][3]
        facility_route2 = route[1][3]

        print(facility_route)
        print(facility_route2)

        self.advance_search.change_facility_in_profile(
            self.data["facility_one"])  # this function works consistently for changing facility.
        self.packageEntry.click_package_entry()
        time.sleep(2)
        stop_name_random = "tc-" + generate_random_string(8)
        barcode1 = self.packageEntry.create_a_package_with_route(customer_name=self.data["customer_name"],
                                                                 forward_branch=self.data["forward_branch"],
                                                                 stop_name=stop_name_random,
                                                                 address_line1=self.data["address_line1"],
                                                                 city=self.data["city"],
                                                                 state=self.data["state"], zip=self.data["zip"],
                                                                 route=facility_route)

        time.sleep(20)
        barcode2 = self.packageEntry.create_a_package_with_route(customer_name=self.data["customer_name"],
                                                                 forward_branch=self.data["forward_branch"],
                                                                 stop_name=stop_name_random,
                                                                 address_line1=self.data["address_line1"],
                                                                 city=self.data["city"],
                                                                 state=self.data["state"], zip=self.data["zip"],
                                                                 route=facility_route2)

        package_count = self.eos_manual_regression_page.getting_packages(facility_route2)
        self.dispatch_dashboard.search_route_and_click2(facility_route)
        self.dispatch_dashboard.move_package_to_another_route(facility_route2)
        self.eos_manual_regression_page.verifying_packages_after_move(facility_route2, package_count)

